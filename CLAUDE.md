...
| 2 — Hooks | Built | 2 PreToolUse safety command hooks (Bash dispatcher + file-path guard) + `permissions.deny` list + Stop/SessionStart comment-cleanup hook (`comment-suspects.mjs`: nominates comments added this turn, bounces the stop once; narration suspects get delete-or-keep, long doc blocks above exports get a compress instruction; model keeps content judgment) + Stop/SessionStart lint kit (`quality-gate.sh` lints changed files against a per-worktree snapshot taken by `lint-baseline.sh`; rules live in `claude/lint/`, never in the repo) + UserPromptSubmit turn-start snapshot (`turn-start.sh`: both Stop hooks skip turns that left the tree unchanged). |
...
| 5 — Orchestration | Built | `Orchestrator` output style (default via `outputStyle`) + `scout`/`patch`/`builder`/`deep` agent roster pinning model+effort tiers. Scale fan-outs stay behind `/fan`. |
