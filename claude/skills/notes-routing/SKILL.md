...
1. Pick the likelier home from the question, but never assume only one. A topic that sounds like "notes" may still be a committed repo doc, and a repo-sounding topic may be a personal Tolaria note.
2. If the first search comes up empty, search the other before concluding anything is lost. Use `search_notes` for Tolaria.
3. If the Tolaria MCP isn't connected in this session, say so rather than reporting that a note doesn't exist.
